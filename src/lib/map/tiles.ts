...
export function renderOfflineTileSvg(z: number, x: number, y: number): string {
  const label = `${z}/${x}/${y}`;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256" viewBox="0 0 256 256" role="img" aria-label="offline map tile">
  ...
  <text x="128" y="118" text-anchor="middle" fill="#5f6f82" font-family="system-ui, -apple-system, Segoe UI, Roboto, sans-serif" font-size="14">نقشه پایه در دسترس نیست</text>
  <text x="128" y="142" text-anchor="middle" fill="#8292a5" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="11">${label}</text>
</svg>`;
}
