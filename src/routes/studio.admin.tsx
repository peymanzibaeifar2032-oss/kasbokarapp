...
function TattooAdminCard({
  ...
  const [time, setTime] = useState(
    request.proposedSlotStart
      ? new Date(request.proposedSlotStart).toLocaleTimeString("en-GB", {
      ...
      !day
        ...
        : bookings.filter(
            (booking) =>
              ...
              new Date(booking.slotStart).toLocaleDateString("en-CA", {
                timeZone: "Asia/Tehran",
              }) === day,
          ),
        ...
        <>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="تاریخ پیشنهادی">
              <JalaliDatePicker value={day} onChange={setDay} label="انتخاب روز از تقویم شمسی" />
            </Field>
...
function ReceiptReview({ request, onChange }: { request: TattooRequest; onChange: () => void }) {
  ...
  async function decide(approved: boolean) {
    setBusy(true);
    try {
      await saveAction("decideTattooReceipt", { requestId: request.id, approved, message });
      toast.success(approved ? "رسید تأیید و نوبت در تقویم قطعی شد." : "رسید رد شد.");
      onChange();
    } catch (err) {
      toast.error(friendlyError(err));
    } finally {
